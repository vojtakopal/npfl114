...
for hidden_layer in args.hidden_layers:
    ...
    model.add(dropout)
model.add(tf.keras.layers.Dense(MNIST.LABELS, kernel_regularizer=regulizer, bias_regularizer=regulizer))
...
def to_categorical(labels):
    return tf.keras.utils.to_categorical(y=labels, num_classes=MNIST.LABELS)
...
model.fit(
    mnist.train.data["images"][:5000], to_categorical(mnist.train.data["labels"][:5000]),
    batch_size=args.batch_size, epochs=args.epochs,
    validation_data=(mnist.dev.data["images"], to_categorical(mnist.dev.data["labels"])),
    callbacks=[tb_callback],
)

test_logs = model.evaluate(mnist.test.data["images"], to_categorical(mnist.test.data["labels"]), batch_size=args.batch_size)
